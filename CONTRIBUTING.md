...
4. Download the config file for the Android app to `ground/src/debug/dev/google-services.json`
...
* App crashes on start with following stacktrace:
  ...
  Solution: Ensure `ground/src/debug/dev/google-services.json` exists and is valid, as per instructions in [Set up Firebase](#set-up-firebase). You may need to perform a clean build in Android Studio by going to Build -> Clean Project.
