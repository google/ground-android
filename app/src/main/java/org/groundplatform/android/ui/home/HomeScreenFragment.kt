...
class HomeScreenFragment : AbstractFragment(), BackPressListener {
  ...
  override fun onCreateView(
    ...
    val contentRoot = createContentRoot()
    val drawerView = createDrawerView()

    drawerLayout.addView(contentRoot)
    drawerLayout.addView(drawerView)

    ensureMapFragmentAdded()
    configureDrawerContent(drawerView)
    configureOverlaysContent(contentRoot.findViewById(R.id.overlays_view))
  ...
  private fun createContentRoot(): ViewGroup {
    ...
    overlaysView.id = R.id.overlays_view
  ...
  private fun createDrawerView(): ComposeView {
  ...
  private fun ensureMapFragmentAdded() {
  ...
  private fun configureDrawerContent(drawerView: ComposeView) {
  ...
  private fun DrawerContent(
    ...
    val closeDrawer: () -> Unit = { scope.launch { drawerLayout.closeDrawer(GravityCompat.START) } }
  ...
  private fun configureOverlaysContent(overlaysView: ComposeView) {
    overlaysView.setContent {
      org.groundplatform.android.ui.theme.AppTheme {
        ...
        androidx.compose.runtime.LaunchedEffect(Unit) {
          homeScreenViewModel.showSignOutDialog.collect { showSignOutDialog.value = true }
        }
