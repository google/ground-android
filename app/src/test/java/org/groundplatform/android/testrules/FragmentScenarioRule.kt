...
class FragmentScenarioRule : ExternalResource() {
  val scenarios = mutableListOf<ActivityScenario<*>>()
  ...
  inline fun <reified T : Fragment> launchFragmentInHiltContainer(
    ...
    hiltActivityScenario(intentData)
      .launchFragment<T>(fragmentArgs, {}) { this.action() }
      .also { scenarios.add(it) }

  /**
   * Launches a fragment in a Hilt enabled ActivityScenario after setting up the NavController for
   * the given [destId]. This is needed for injecting view models that are scoped to graph.
   */
