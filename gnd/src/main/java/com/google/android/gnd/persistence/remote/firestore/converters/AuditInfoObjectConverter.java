...
public class AuditInfoObjectConverter {
  /**
   ...
   */
  @NonNull
  public static AuditInfo toAuditInfo(@Nullable AuditInfoObject doc) {
    if (doc == null || doc.getClientTimeMillis().isEmpty()) {
      return AuditInfo.builder()
          .setUser(UserObjectConverter.UNKNOWN_USER)
          .setClientTimeMillis(new Date(0))
          .build();
    }
    return AuditInfo.builder()
        .setUser(UserObjectConverter.toUser(doc.getUser()))
        .setClientTimeMillis(doc.getClientTimeMillis().get().toDate())
        .setServerTimeMillis(doc.getServerTimeMillis().map(Timestamp::toDate))
        .build();
  }

  public static AuditInfoObject fromMutationAndUser(Mutation mutation, User user) {
    return AuditInfoObject.builder()
        .setUser(UserObjectConverter.fromUser(user))
