...
/** Metadata and raw image data for a single tile. */
class MogTile(val metadata: MogTileMetadata, val data: ByteArray) {
  ...
  /** Combines JPEG tables and image data into a valid standalone JFIF (JPEG) byte array. */
  fun buildJfifFile(): ByteArray =
    START_OF_IMAGE +
      app0Segment() +
      jpegTablesData +
  ...
  private fun app0Segment() =
    APP0_MARKER +
      ...
      JFIF_IDENTIFIER.encodeToByteArray() +
      0x00.toByte() +
