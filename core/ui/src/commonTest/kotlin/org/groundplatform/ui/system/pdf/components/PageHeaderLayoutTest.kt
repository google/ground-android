...
class PageHeaderLayoutTest {
  ...
  fun `column X positions span the usable width with gaps between them`() {
    ...
    assertTrue(
      3 * width + 2 * headerColumnGap <= usableWidth,
      "Three columns plus gaps must fit within USABLE_WIDTH",
    )
  }
  ...
  fun `compute places survey column labels and values at LEFT_X`() {
    ...
    assertEquals(
      PdfRenderer.Offset(PageHeaderLayout.LEFT_X, 0f),
      layout.leftColumn.labelOffset,
    )
    assertEquals(
      PdfRenderer.Offset(PageHeaderLayout.LEFT_X, 10f + lineSpacing),
  ...
  fun `compute places job column labels and values at CENTER_X`() {
    ...
    assertEquals(
      PdfRenderer.Offset(PageHeaderLayout.CENTER_X, 0f),
      layout.centerColumn.labelOffset,
    )
    assertEquals(
      PdfRenderer.Offset(PageHeaderLayout.CENTER_X, 10f + lineSpacing),
  ...
  fun `compute places timestamp at RIGHT_X with the same top as labels`() {
    ...
    assertEquals(PdfRenderer.Offset(PageHeaderLayout.RIGHT_X, 50f), layout.rightTextOffset)
