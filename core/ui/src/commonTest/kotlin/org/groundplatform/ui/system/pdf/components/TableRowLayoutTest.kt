...
class TableRowLayoutTest {
  ...
  fun `totalHeight picks the taller content height`() {
    ...
    val tallerImageRight =
      TableRowLayout.totalHeight(
        leftTextHeight = 20f,
        rightTextHeight = 0f,
        rightImageSize = PdfRenderer.Size(width = 100f, height = 80f),
      )
  ...
  fun `compute always places left text at the row's top-left content area`() {
    ...
    assertEquals(PdfRenderer.Offset(margin + cellPadding, 100f + cellPadding), layout.leftTextOffset)
  }
  ...
  fun `compute places right text at the right cell's top`() {
    ...
    assertEquals(PdfRenderer.Offset(rightCellX, 50f + cellPadding), layout.rightTextOffset)
    assertNull(layout.rightImageFrame)
  }

  @Test
  fun `compute places image at the right cell's top`() {
    val imageSize = PdfRenderer.Size(width = 80f, height = 60f)
    val layout =
      TableRowLayout.compute(
        rowTop = 50f,
        leftTextHeight = 20f,
  ...
  fun `compute totalHeight matches the static helper`() {
    val left = 30f
    val right = 20f
    val image = PdfRenderer.Size(width = 80f, height = 60f)
