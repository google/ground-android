 ...
 *
 *
 * Implementations are expected to execute each method in this class as a single atomic
 * transaction, and must ensure all subscriptions are run in a background thread (i.e., not the
 * Android main thread).
 *
 *
 * Note that long-lived streams return the full set of entities on each emission rather than
...
interface LocalDataStore {
    ...
    /**
     * Applies the specified [SubmissionMutation] to the local data store, appending the
     * mutation to the local queue for remote sync.
     */
    ...
    fun apply(mutation: SubmissionMutation): @Cold Completable
     ...
     */
    fun getLocationsOfInterestOnceAndStream(survey: Survey): @Cold(terminates = false) Flowable<ImmutableSet<LocationOfInterest>>
