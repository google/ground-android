...
import static com.google.android.ground.rx.Nil.NIL;
import static com.google.android.ground.rx.RxCompletable.toBooleanSingle;
...
public class HomeScreenViewModel extends AbstractViewModel {
  ...
  public void addPolygonOfInterest(LocationOfInterest<Polygon> areaOfInterest) {
    getActiveSurvey()
        .map(Survey::getId)
        .ifPresentOrElse(
            surveyId ->
                addLocationOfInterestRequests.onNext(
                    locationOfInterestRepository.newPolygonOfInterestMutation(
                        ...
                        areaOfInterest.getGeometry().getVertices(),
                        new Date())),
            () -> {
              throw new IllegalStateException("Empty survey");
            });
