...
class CogTileProvider(private val tiledCogCollection: TiledCogCollection) : TileProvider {
  override fun getTile(x: Int, y: Int, zoom: Int): Tile? = runBlocking {
    ...
    try {
      val tile = tiledCogCollection.getTile(tileCoordinates) ?: return@runBlocking null
      Tile(tile.width.toInt(), tile.height.toInt(), tile.imageBytes)
    } catch (e: Throwable) {
      Timber.d(e, "Failed to fetch tile at $tileCoordinates")
      null
    }
  }
}
