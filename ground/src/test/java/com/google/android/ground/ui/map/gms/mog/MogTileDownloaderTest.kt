...
class MogTileDownloaderTest {
  ...
  fun downloadTiles() = runBlocking {
    ...
    assertThat(numRequests).isEqualTo(205)
    assertThat(numFiles).isEqualTo(10)
    assertThat(numTiles).isEqualTo(33412)

    // Select a random request instead of fetching all tiles.
    val filteredRequests = listOf(requests.random())

    // Download!
    val downloadTimeMillis = measureTimeMillis {
      MogTileDownloader(client, "/tmp/tiles").downloadTiles(filteredRequests)
    }
    println("Downloaded ${filteredRequests.size} in %.1fs".format(downloadTimeMillis / 1000.0))

    assertThat(downloadTimeMillis).isGreaterThan(0) // 0 seconds
    assertThat(downloadTimeMillis).isLessThan(5000) // 5 seconds
  }
}
