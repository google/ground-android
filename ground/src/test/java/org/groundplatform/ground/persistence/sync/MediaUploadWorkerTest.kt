 ...
 */
package org.groundplatform.ground.persistence.sync
...
import org.groundplatform.android.persistence.sync.MediaUploadWorker
...
class MediaUploadWorkerTest : BaseHiltTest() {
  ...
  fun doWork_succeedsOnExistingPhoto() = runWithTestDispatcher {
    ...
    localSurveyStore.insertOrUpdateSurvey(org.groundplatform.ground.persistence.sync.MediaUploadWorkerTest.Companion.TEST_SURVEY)
    localLocationOfInterestStore.insertOrUpdate(org.groundplatform.ground.persistence.sync.MediaUploadWorkerTest.Companion.TEST_LOI)
  ...
  fun doWork_propagatesFailures() = runWithTestDispatcher {
    ...
    val delta = buildList {
      ...
      add(ValueDelta(org.groundplatform.ground.persistence.sync.MediaUploadWorkerTest.Companion.PHOTO_TASK_ID, PHOTO, PhotoTaskData("some/path/does_not_exist.jpg")))
    }
    val updatedMutation =
      mutation.copy(
        deltas = delta,
  ...
  private suspend fun createSubmissionMutation(photoName: String? = null): SubmissionMutation {
    val photo =
      userMediaRepository.savePhoto(
        ...
        org.groundplatform.ground.persistence.sync.MediaUploadWorkerTest.Companion.TEST_PHOTO_TASK.id,
      )

    return org.groundplatform.ground.persistence.sync.MediaUploadWorkerTest.Companion.SUBMISSION_MUTATION.copy(
      job = org.groundplatform.ground.persistence.sync.MediaUploadWorkerTest.Companion.TEST_JOB,
      deltas = listOf(ValueDelta(org.groundplatform.ground.persistence.sync.MediaUploadWorkerTest.Companion.PHOTO_TASK_ID, PHOTO, PhotoTaskData(photoName ?: photo.name))),
    )
  }
  ...
  companion object {
    ...
    private val TEST_JOB = FakeData.JOB.copy(tasks = mapOf(org.groundplatform.ground.persistence.sync.MediaUploadWorkerTest.Companion.PHOTO_TASK_ID to org.groundplatform.ground.persistence.sync.MediaUploadWorkerTest.Companion.TEST_PHOTO_TASK))

    private val TEST_LOI = FakeData.LOCATION_OF_INTEREST.copy(job = org.groundplatform.ground.persistence.sync.MediaUploadWorkerTest.Companion.TEST_JOB)
    private val TEST_SURVEY = FakeData.SURVEY.copy(jobMap = mapOf(org.groundplatform.ground.persistence.sync.MediaUploadWorkerTest.Companion.TEST_JOB.id to org.groundplatform.ground.persistence.sync.MediaUploadWorkerTest.Companion.TEST_JOB))
    ...
    val SUBMISSION_MUTATION =
      SubmissionMutation(
        ...
        deltas = listOf(ValueDelta(org.groundplatform.ground.persistence.sync.MediaUploadWorkerTest.Companion.PHOTO_TASK_ID, PHOTO, PhotoTaskData("foo/${org.groundplatform.ground.persistence.sync.MediaUploadWorkerTest.Companion.PHOTO_TASK_ID}.jpg"))),
      )
  }
}
